...
public class OI {
    ...
    public OI() {
        ...
        // D8 free
        // D9 free
        D10.whenPressed(new VisionWriteCalibrationImages());

        //Custom control interface for the shooter button allocations
        M1.whenPressed(new CatapultFireAndRewind());
        M2.whenPressed(new CatapultReleaseAndReturnLatch());
        M3.whileHeld(new IntakeRollerForward());
        M3.whenReleased(new IntakeRollerStop());
        M4.whileHeld(new IntakeRollerReverse());
        M4.whenReleased(new IntakeRollerStop());
        M5.whileHeld(new CatapultWindWinchForward());
        M5.whenReleased(new CatapultWindWinchStop());
        M6.whileHeld(new CatapultWindWinchReverse());
        M6.whenReleased(new CatapultWindWinchStop());
        // M7 free
        // M8 free
        M9.whenPressed(new StingerDeploy());
        M10.whenPressed(new StingerRetract());
        M11.whenPressed(new IntakePositionDeploy());
        M12.whenPressed(new IntakePositionRetract());
    }
