...
public class Vision extends Subsystem {
    ...
    final double VIEW_ANGLE = 67;		//Axis M1013 originally said it was 49 degrees; vertical angle is 51
    ...
    public void getImages() {
        try {
            ...
            thresholdImage = image.thresholdHSV(100, 175, 0, 255, 200, 255);   // keep only green objects
        ...
        try {
            ...
            if (filteredImage.getNumberParticles() > 0) {
                ...
                if (verticalTargetCount > 0) {
                    ...
                    targetDistance = computeDistance(filteredImage, distanceReport, target.verticalIndex);
                }
            }
        } catch (NIVisionException ex) {
            ex.printStackTrace();
    ...
    public void writeImages(String suffix) {
        try {
            if(suffix == null) {
                suffix = "";
            }
            image.write("/rawImage" + suffix + ".bmp");
            thresholdImage.write("/threshold" + suffix + ".bmp");
            filteredImage.write("/filteredImage" + suffix + ".bmp");
        } catch (NIVisionException ex) {
            ex.printStackTrace();
        }
    }
    
    public void takeCalibrationImages() {
        calCount++;
        getImages();
        writeImages("_cal_" + calCount);
    }
